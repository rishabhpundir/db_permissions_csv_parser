...
from postgres_users_creation import (
    load_parameters,
    create_user,
    create_role,
    create_schema,
...
def test_load_parameters(tmp_path: Path):
    ...
    A temporary CSV file with test data is created and the parameters are checked for loading correctly.
    """
    logging.info("Starting test_load_parameters")
    test_data = [
        ["Key1", "value1"],
        ["Key2", "value2, value3"],
        ["InvalidRow"]  # Row with insufficient number of columns
    ]
    file_path = tmp_path / "test_params.csv"
    with file_path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerows(test_data)
    params = load_parameters(str(file_path))
